...
a="C:\\Users\\asbar\\OneDrive - Universidad de los Andes\\Carrera\\Séptimo semestre 2023-1\\ANALITICA\P2-actd\\P2\\processed.cleveland_repaired.csv"

df = pd.read_csv(a)
#conectar con la base de datos
engine = create_engine('postgresql://postgres:Sofia2020@sofia.cywkx5wmz9xp.us-east-1.rds.amazonaws.com:5432/postgres')
table_name = 'bd'
# cargar datos a la tabla en la base de datos
df.to_sql(table_name, engine, if_exists='replace', index=False)

with engine.connect() as conn:
    result = conn.execute(text("SELECT * FROM bd")).fetchall()

datos_colesterol=[]
for row in result:
    datos_colesterol.append(row[4])

datos_edad=[]
for row in result:
    datos_edad.append(row[0])

datos_thalach=[]
for row in result:
    datos_thalach.append(row[7])

datos_fbs=[]
for row in result:
    datos_fbs.append(row[5])
...
b="C:\\Users\\asbar\\OneDrive - Universidad de los Andes\\Carrera\\Séptimo semestre 2023-1\\ANALITICA\\P2-actd\\P2\\heart_disease_modified2.csv"
df = pd.read_csv(b)
...
corr_coef = np.corrcoef(datos_edad, datos_thalach)[0, 1]
...
cuenta = [datos_fbs.count(0), datos_fbs.count(1)]
...
servicio_1 = html.Div([
    ...
    html.Br(),
    html.Button('Enviar', id='button', style={'color': '#273746','background-color': '#E5E8E8'}),
        html.Br(),
        html.Div(id='output'
                  )   
...
html.P("3. No fumar: Fumar es un factor de riesgo importante para las enfermedades cardiovasculares. Dejar de fumar puede reducir significativamente este riesgo")
    ...
    html.Div([
html.P("A continuación puede observar una infografía diseñada por 'Organizaciones Saludables'")
    ]),
    html.Br(),
     html.Div([
        html.Img(src="https://img.interempresas.net/fotos/3185508.jpeg",height="550px")])
...
sofia="https://media.licdn.com/dms/image/D5603AQFfpKhMNX6srw/profile-displayphoto-shrink_400_400/0/1683133216510?e=1688601600&v=beta&t=LXio1TnM5FhBOYnFu0iG9LquqlRA5fmeaIOrcma1crg"
juan_jo= "https://pin.it/5edjPXr"
sebas="https://media.licdn.com/dms/image/D4E03AQFnhcCW3wHOQw/profile-displayphoto-shrink_800_800/0/1671024051405?e=1688601600&v=beta&t=VlxvYsuWKO2zTue-nZ-KoO30U0mYZAHqLLZeaTxmVBY"


description_1 = html.Ul([
    html.Li("Adriana Sofia Barrera"),
    html.Li("as.barrera@uniandes.edu.co"),
    html.Li("202011470")
])
...
description_2 = html.Ul([
    ...
    html.Li("j.florezr@uniandes.edu.co"),
    html.Li("202011038")
])
...
description_3 = html.Ul([
    ...
    html.Li("js.hernandezp@uniandes.edu.co"),
    html.Li("201715595")
])

image_list = html.Div([
    html.Div([
        html.Img(src=sofia, height="145px",style={'border-radius': '50%'}),
        html.Div(description_1, style={"display": "inline-block", "vertical-align": "top",'fontSize': 20,})
    ], style={'display': 'flex', 'flex-direction': 'row'}),
    
    html.Div([
        html.Img(src=image_url, height="145px",style={'border-radius': '50%'}),
        html.Div(description_2, style={"display": "inline-block", "vertical-align": "top",'fontSize': 20})
    ], style={'display': 'flex', 'flex-direction': 'row'}),
    
    html.Div([
        html.Img(src=sebas, height="145px",style={'border-radius': '50%'}),
    ...
    if pathname== '/servicio-1':
        if n_clicks:
            ...
            if proba[0]>0.5:
                ...
                recomendacion="Tienes una probabilidad baja, lo que significa que tu corazón esta sano. Sin embargo, sigue cuidándolo con bueno hábitos"

            return html.Div([
                html.Br(),
                html.H4("Tu probabilidad de tener una enfermedad cardiovascular es de :" +str(round(proba[0]*100,1))+ "%",style={'backgroundColor': '#EBF5FB', 'color': '#1B4F72','padding': '20px'}),
                html.H5(recomendacion,style={'backgroundColor': '#EBF5FB', 'color': '#1B4F72','padding': '20px'})
                             ])
